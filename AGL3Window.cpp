...
{
   double wait_time = static_cast<double>(frame_time) - (glfwGetTime() - prev_time);
   if (wait_time>0.0)
       usleep(static_cast<unsigned int>(1000000 * wait_time)); //before swapBuf.. and time sync 60Hz..
...
void AGLWindow::SetFullScreen(bool fullscreen)
{
   ...
   if (fullscreen)
   {
      ...
      const GLFWvidmode * mode = glfwGetVideoMode(monitor);
      glfwSetWindowMonitor(window, monitor, 0, 0, mode->height, mode->height, 0);
   }else
   {
